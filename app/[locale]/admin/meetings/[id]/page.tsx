...
export default function AdminMeetingDetailPage() {
  ...
  const buildTimelineEvents = (): Array<{
    id: string
    timestamp: string
    ...
    if (meeting.admin_reviews && meeting.admin_reviews.length > 0) {
      meeting.admin_reviews.forEach((review, index) => {
        const status: 'approved' | 'rejected' | 'needs_revision' = 
          review.review_decision === 'approve' ? 'approved' :
    ...
    if (meeting.status !== 'pending') {
      const status: 'approved' | 'rejected' = meeting.status === 'qualified' ? 'approved' : 'rejected'
      events.push({
        id: 'client-decision',
                    ...
                    <div key={review.id} className="border border-gray-200 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-xs text-gray-500">
                          {new Date(review.created_at).toLocaleString(locale === 'sv' ? 'sv-SE' : 'en-US')}
                        </span>
                      </div>
                      {review.review_notes && (
