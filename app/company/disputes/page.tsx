...
export default function DisputeCenterPage() {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-gray-900">Dispute Center</h1>
        <p className="text-sm text-gray-500 mt-1">Manage disputes for your meetings</p>
      </div>
            ...
            <div key={dispute.id} className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <span className="text-xs text-gray-400">
                  {new Date(dispute.created_at).toLocaleDateString('sv-SE')}
                </span>
              </div>
              ...
              {dispute.resolution && (
                <div className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  <label className="text-xs font-medium text-gray-500 uppercase mb-1 block">Admin Resolution</label>
                  <p className="text-sm text-gray-700">{dispute.resolution}</p>
                  {dispute.resolved_at && (
                    <p className="text-xs text-gray-500 mt-2">
                      Resolved: {new Date(dispute.resolved_at).toLocaleString('sv-SE')}
                    </p>
                  )}
                </div>
              )}
